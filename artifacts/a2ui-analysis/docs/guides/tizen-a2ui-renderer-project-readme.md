...
- Controller: `E_SURFACE_NOT_FOUND`, `E_SURFACE_DELETED`, `E_PENDING_OVERFLOW`, `E_PENDING_EXPIRED`, `E_FUNCTION_NAME_REQUIRED`, `E_FUNCTION_CALL_DUPLICATE`, `E_FUNCTION_RESPONSE_ORPHAN`, `E_FUNCTION_RESPONSE_LATE`, `E_FUNCTION_SURFACE_MISMATCH`, `E_FUNCTION_TIMEOUT`
- Runtime: `E_RUNTIME_OPERATION_FAILED`, `E_RUNTIME_ADAPTER_NOT_CONFIGURED`, `E_RUNTIME_ADAPTER_INTEGRATION_INVALID`
...
## Operator Runbooks
- integration checklist: `../operations/tizen-runtime-integration-checklist.md`
- incident playbook: `../operations/tizen-runtime-incident-playbook.md`
...
- 스트림 복원력 검증:
  - 대용량 batch + malformed segment + cancel/late response 혼합 입력에서 후속 정상 메시지가 처리되어야 한다.
  - corrupted tail 이후 `Flush()`에서 `E_PARSE_INCOMPLETE_JSON`이 보고되어야 한다.
- 구조화 로그 검증:
  - 주요 에러 로그에 `source`, `error_component`, `error_kind`, `error_code`, `error_message`, `integration_path` 필드가 일관되게 존재해야 한다.
  - runtime adapter 에러 로그에 `operation`, `adapter_type`, `bridge_type` 필드가 포함되어야 한다.
- 배포 전 테스트:
  - `/usr/local/share/dotnet/dotnet test` 전체 통과
  - mixed version + malformed recovery 테스트 통과
  - composite resilience 테스트(long stream + malformed + cancel + late response) 통과
