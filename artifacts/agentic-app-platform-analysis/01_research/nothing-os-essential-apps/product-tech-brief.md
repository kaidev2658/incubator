...
### 2.4 PWA 기반 여부 점검 (2026-03-11)
- `https://playground.nothing.tech/apps` 정적 응답 기준으로는 `<link rel="manifest">`가 확인되지 않음.
- 일반적인 PWA 엔드포인트(`/manifest.json`, `/site.webmanifest`, `/sw.js`) 접근 시 유효 manifest/service worker 파일이 아닌 HTML 응답이 반환됨.
- 번들 내 `serviceworker` 문자열은 React preload 타입 열거 문맥으로, 서비스워커 등록 코드의 직접 근거는 아님.
- 결론: **현재 공개 페이지 관찰만으로는 "PWA 기반"을 Confirmed로 보기 어려움**. **[Open]**
- 권고: 브라우저 DevTools(Application 탭)에서 실제 서비스워커 등록/매니페스트 로딩을 런타임 검증 후 재판정.
...
## 9) 보완 리서치 (Essential Apps 중심)
### 9.1 앱 모델 정리
- Essential Apps는 Nothing Playground에서 생성/수정/배포되는 **홈스크린 중심 마이크로앱 모델**로 해석하는 것이 가장 일관적이다. **[Confirmed/Likely]**
- 전통적 “독립 APK 앱스토어 배포”보다, 플랫폼이 제공하는 생성 파이프라인을 거쳐 결과물을 런타임 표면(홈스크린)으로 반영하는 구조에 가깝다. **[Likely]**

### 9.2 앱 생성(개발) 방식
- 입력: 자연어 프롬프트 기반 앱 초안 생성 **[Confirmed]**
- 반복: draft 상태에서 수정/개선 루프 수행 **[Confirmed]**
- 변경 전략: full-regenerate 대신 partial update 우선 **[Confirmed]**

### 9.3 배포 방식
- 웹 Playground에서 배포 트리거 후 디바이스 반영 **[Confirmed]**
- 상태 모델: draft/live 구분, 이력 기반 되돌리기(rollback) 제공 **[Confirmed]**

### 9.4 구동(Runtime) 방식
- 런타임의 본질은 “가벼운 UI 표면 + 제한된 capability 실행”이다. **[Likely]**
- 권한 허용 범위 안에서만 동작하며, 고위험 capability는 단계적으로 개방한다는 운영정책이 확인된다. **[Confirmed/Likely]**

### 9.5 위젯 앱 모델·언어 보완
- 앱 표면은 위젯형 UX에 가까워, 선언형 UI/액션 스키마 기반 설계가 적합하다. **[Likely]**
- 구현 언어는 런타임 종속(예: Android 영역 Kotlin/Java 가능)이나, 생성 산출물 포맷은 JSON/DSL처럼 중립 형태로 두는 편이 이식성과 운영성에 유리하다. **[Likely]**

## 10) 엔지니어링 보완 포인트 (Essential Apps 관점)
- capability matrix: `stable / beta / planned`를 앱 기능과 1:1 매핑
- 배포 계약 명시: `manifest + ui_schema + action_schema + permission policy`
- partial update 단위 정의: UI/데이터/액션 레이어별 hot-swap 범위 고정
- rollback SLO 정의: 실패율·지연·권한거부 급증 시 자동 복원 트리거
- 관측성 지표: deploy success rate, rollback rate, first-render latency, action completion rate

## 11) Tizen PoC 직접 매핑 (Essential Apps-only)
- SCN-01을 Essential Apps 패턴으로 고정:
  - Prompt -> draft 생성 -> 수정 -> live 배포 -> 홈 반영 -> rollback 검증
- PoC 산출물 포맷은 런타임 중립(JSON/DSL), 실행부는 C# 우선 유지
- 고위험 capability(카메라/마이크/네트워크)는 단계 개방 원칙으로 fail-safe 기본값 유지

## 12) Source Pointers
### Primary (Nothing 공식/지정)
